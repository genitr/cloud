...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == "list":
            logger.debug(
                f"Пользователь {user.username} запросил список, возвращаем только себя"
            )
    ...
    def me(self, request):
        ...
        if request.method == "GET":
            ...
            logger.info(
                f"Пользователь {user.username} (ID: {user.id}) обновляет свой профиль"
            )
            ...
            logger.info(
                f"Профиль пользователя {user.username} (ID: {user.id}) успешно обновлен"
            )
    ...
    def login(self, request):
        """
        Вход в систему (использует сессии)
        ...
        logger.info(
            f"Пользователь {user.username} (ID: {user.id}) успешно вошел в систему."
        )
    ...
    def logout(self, request):
        ...
        logger.info(
            f"Пользователь {user.username} (ID: {user.id}) успешно вышел из системы"
        )
    ...
    def storage(self, request, pk=None):
        ...
        logger.info(
            f"Запрос информации о хранилище пользователя {user.username} (ID: {user.id}) от {request.user.username}"
        )
        ...
        logger.debug(
            f"Статистика хранилища для {user.username}: файлов={files_count}, папок={folders_count}, размер={total_size} байт"
        )

        # Статистика по типам файлов
        files_by_type = (
            files.values("content_type")
        ...
        return Response(
            {
                ...
                "storage_url": (
                    f"/media/{user.storage_path}" if user.storage_path else None
                ),
                "statistics": {
    ...
    def toggle_active(self, request, pk=None):
        ...
        if user == request.user:
            logger.warning(
                f"Пользователь {admin.username} попытался заблокировать самого себя"
            )
            return Response(
        ...
        if not user.is_active:
            logger.info(f"Админ {admin.username} заблокировал пользователя {user.username}")
        else:
            logger.info(f"Админ {admin.username} разблокировал пользователя {user.username}")
    ...
    def stats(self, request):
        ...
        if not request.user.is_staff:
            logger.warning(
                f"Пользователь {request.user.username} попытался получить статистику без прав администратора"
            )
            return Response(
        ...
        logger.debug(
            f"Статистика: всего={total_users}, активных={active_users}, сотрудников={staff_users}, новых={new_users}"
        )
    ...
    def make_admin(self, request, pk=None):
        ...
        if user == request.user:
            logger.warning(
                f"Пользователь {admin.username} попытался назначить себя администратором через make_admin"
            )
            return Response(
        ...
        logger.info(
            f"Админ {admin.username} назначил пользователя {user.username} (ID: {user.id}) администратором"
        )
    ...
    def remove_admin(self, request, pk=None):
        ...
        if user == request.user:
            logger.warning(
                f"Пользователь {admin.username} попытался снять права администратора с самого себя"
            )
            return Response(
        ...
        logger.info(
            f"Админ {admin.username} снял права администратора с пользователя {user.username} (ID: {user.id})"
        )
    ...
    def make_superuser(self, request, pk=None):
        ...
        if user == request.user:
            logger.warning(
                f"Пользователь {admin.username} попытался назначить себя суперпользователем"
            )
            return Response(
                {"error": "Нельзя изменить свои права через этот эндпоинт"},
                status=status.HTTP_400_BAD_REQUEST,
        ...
        logger.info(
            f"Админ {admin.username} назначил пользователя {user.username} (ID: {user.id}) суперпользователем"
        )
    ...
    def perform_destroy(self, instance):
        ...
        logger.warning(
            f"Удаление пользователя {instance.username} (ID: {instance.id}) администратором {self.request.user.username}"
        )

        instance.delete()
        logger.info(
            f"Пользователь {instance.username} (ID: {instance.id}) успешно удален"
        )

    def create(self, request, *args, **kwargs):
        """
        Переопределяем create для регистрации.
